...
export default function PathFinderPage() {
  ...
  const findPath = (e) => {
    ...
    api
      ...
      .catch((err) =>
        setPath({ status: "error", data: null, error: err.message }),
      );
  };
    ...
    <div className="mx-auto max-w-5xl px-6 py-10">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-ink/55">
          Shortest connection between any two developers, traced through shared
          projects and skills.
        </p>
      </div>
      ...
      {from && to && from === to && (
        <EmptyState
          title="Pick two different developers"
          hint="A path needs two distinct endpoints."
        />
      )}

      {path.status === "loading" && (
        <LoadingState label="Searching the graph…" />
      )}
      {path.status === "error" && (
        <ErrorState message={path.error} onRetry={findPath} />
      )}
      {path.status === "ready" && path.data.nodes.length === 0 && (
        <EmptyState
          title="No connection found"
          hint="These two developers aren't connected in the graph yet."
        />
      )}
      ...
      {path.status === "ready" && path.data.nodes.length > 0 && (
        <div className="rounded-panel border border-line bg-white p-8 overflow-x-auto">
              ...
              <div key={node.id} className="flex items-center">
                ...
                {i < path.data.relationships.length && (
                  <div className="w-16 flex items-center px-1">
                    <TraceLine
                      label={path.data.relationships[i].type}
                      animated
                    />
                  </div>
                )}
          ...
          <p className="mt-6 text-center font-mono text-[11px] text-ink/40">
            {path.data.relationships.length} hop
            {path.data.relationships.length === 1 ? "" : "s"}
          </p>
        </div>
      )}
    </div>
